...
public class Api {
    ...
    public long resolveScreenName(String name) throws IOException, JSONException, KException{
        Params params = new Params("utils.resolveScreenName");
        params.put("screen_name", name);
        JSONObject root = sendRequest(params);
        JSONObject response=root.optJSONObject("response");
        Long result = response.getLong("object_id");
    ...
    public ArrayList<Long> getFriends(Long user_id, String fields, Integer lid, String captcha_key, String captcha_sid) throws IOException, JSONException, KException{
    ...
    public ArrayList<Photo> getPhotosById(String photos, Integer extended, Integer photo_sizes) throws IOException, JSONException, KException{
        ...
        //uid_pid_accesskey, uid_pid_accesskey
    ...
    //http://vk.com/dev/video.add
    //todo longpoll
    public String sendLongPollRequest(String server, String key, Long ts) throws IOException, JSONException, KException {
        ...
        for(int i=1;i<=MAX_TRIES;++i){
            try{
                if(i!=1)
                    Log.i(TAG, "try "+i);
                response = sendRequestInternal(url, body, false);
                break;
            }catch(javax.net.ssl.SSLException ex){
                processNetworkException(i, ex);
            }catch(java.net.SocketException ex){
    ...
    public ArrayList<Long> getGroupsMembers(long gid, Integer count, Integer offset, String sort, String filter) throws IOException, JSONException, KException {
    ...
    //http://vk.com/dev/groups.getMembers
    public ArrayList<User> getGroupsManagers(long gid, Integer count, Integer offset, String sort) throws IOException, JSONException, KException {
        Params params = new Params("groups.getMembers");
        params.put("group_id", gid);
        params.put("count", count);
        params.put("offset", offset);
        params.put("sort", sort); //id_asc, id_desc, time_asc, time_desc
        params.put("filter", "managers"); //managers
        JSONObject root = sendRequest(params);
        JSONObject response=root.getJSONObject("response");
        JSONArray array=response.optJSONArray("users");
        ArrayList<User> users=new ArrayList<User>();
    ...
    public ArrayList<Document> getDocsById(String doc) throws IOException, JSONException, KException {
        Params params = new Params("docs.getById");
        params.put("docs", doc);
        JSONObject root = sendRequest(params);
        JSONArray response=root.optJSONArray("response");
        return Document.parseDocs(response);
    }
