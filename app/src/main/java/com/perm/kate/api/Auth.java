...
public class Auth {
    ...
    public static String getUrl(String api_id, String settings){
        //https://oauth.vk.com/authorize?client_id=4485671&revoke=1&response_type=token&display=mobile&redirect_uri=https://oauth.vk.com/blank.html&scope=notify,friends,photos,audio,video,docs,status,notes,pages,wall,groups,messages,offline,notifications
        String url="https://oauth.vk.com/authorize?client_id="+api_id+"&display=mobile&scope="+settings+"&revoke=1&redirect_uri="+URLEncoder.encode(redirect_url)+"&response_type=token"
