...
public class ThematicsProcessor implements Command {
    ...
    public void load() {
        ...
        if(fileText == null) {
            log("! Ошибка прочтения файла "+thematics+" обработчика: " + resourceFileReader.getFilePath());
            return;
        }
    ...
    String save(){
        boolean result = resourceFileReader.writeFile(patterns.save() + "\n" + answers.save());
        if(result){
            log(". Сохранение данных обработчика ("+patterns.size() + " шаблонов и " + answers.size() + " ответов) выполнено в " + resourceFileReader.getFilePath());
            return ". Сохранение данных обработчика ("+patterns.size() + " шаблонов и " + answers.size() + " ответов) выполнено в " + resourceFileReader.getFilePath() + "\n";
        }
        else{
            log("! Ошибка сохранения тематического обработчика в " + resourceFileReader.getFilePath());
            return "! Ошибка сохранения тематического обработчика в " + resourceFileReader.getFilePath();
        }
    }
    ...
    private class Status implements Command{
        ...
        public String process(String input, Long senderId) {
            ...
            if(commandParser.getWord().equals("status")){
                return  "Количество шаблонов тематического процессора "+ resourceFileReader.getFileName()+" : "+patterns.size()+"\n" +
                        "Количество ответов тематического процессора "+ resourceFileReader.getFileName()+" : "+answers.size()+"\n" +
                        "Расположение файла базы данных тематического процессора "+ resourceFileReader.getFileName()+" : "+ resourceFileReader.getFilePath()+"\n";
            }
            return "";
        }
    }
    ...
    private class RemPatt implements Command{
        ...
        public String getHelp() {
            return "[ Удалить "+thematics+" шаблон ]\n" +
                    "---| botcmd "+thematics+" rempatt <текст шаблона>\n\n";
        }
    ...
    private class RemAnsw implements Command{
        ...
        public String getHelp() {
            return "[ Удалить "+thematics+" ответ ]\n" +
                    "---| botcmd "+thematics+" remansw <текст ответа>\n\n";
        }
    ...
    private class AddPatt implements Command{
        ...
        public String getHelp() {
            return "[ Добавить "+thematics+" шаблон ]\n" +
                    "---| botcmd "+thematics+" addpatt <текст шаблона>\n\n";
        }
    ...
    private class AddAnsw implements Command{
        ...
        public String getHelp() {
            return "[ Добавить "+thematics+" ответ ]\n" +
                    "---| botcmd "+thematics+" addansw <текст ответа>\n\n";
        }
